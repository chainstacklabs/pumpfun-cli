...
def _setup_mocks(MockClient, MockIDL, state, *, side: str = "buy"):
    """Set up mocked RPC client and IDL for buy or sell tests."""
    client = AsyncMock()
    ...
    idl.get_instruction_discriminators.return_value = {side: b"\x00" * 8}
...
async def test_buy_standard_token2022(tmp_keystore):
    ...
    with (
        ...
        _setup_mocks(MockClient, MockIDL, _bc_state())
...
async def test_buy_mayhem_token(tmp_keystore):
    ...
    with (
        ...
        _setup_mocks(MockClient, MockIDL, _bc_state(is_mayhem=True))
...
async def test_sell_standard_no_cashback(tmp_keystore):
    ...
    with (
        ...
        _setup_mocks(MockClient, MockIDL, _bc_state(is_cashback=False), side="sell")
        mock_build.return_value = []
...
async def test_sell_mayhem_token(tmp_keystore):
    ...
    with (
        ...
        _setup_mocks(MockClient, MockIDL, _bc_state(is_mayhem=True), side="sell")
        mock_build.return_value = []
...
async def test_sell_cashback_token(tmp_keystore):
    ...
    with (
        ...
        _setup_mocks(MockClient, MockIDL, _bc_state(is_cashback=True), side="sell")
        mock_build.return_value = []
...
async def test_sell_mayhem_cashback_token(tmp_keystore):
    """BC-SELL-4: Mayhem + cashback sell — both flags passed through."""
    with (
        patch("pumpfun_cli.core.trade.RpcClient") as MockClient,
        patch("pumpfun_cli.core.trade.IDLParser") as MockIDL,
        patch("pumpfun_cli.core.trade.build_sell_instructions") as mock_build,
        ...
        _setup_mocks(MockClient, MockIDL, _bc_state(is_mayhem=True, is_cashback=True), side="sell")
        mock_build.return_value = []
...
async def test_sell_legacy_spl_token(tmp_keystore):
    ...
    with (
        ...
        _setup_mocks(MockClient, MockIDL, _bc_state(), side="sell")
        mock_build.return_value = []
...
async def test_buy_graduated_token_error(tmp_keystore):
    """BC-BUY-8: Buying a graduated token returns error."""
    with (
        patch("pumpfun_cli.core.trade.RpcClient") as MockClient,
        patch("pumpfun_cli.core.trade.IDLParser") as MockIDL,
        _patch_token_program(TOKEN_2022_PROGRAM),
    ):
        _setup_mocks(MockClient, MockIDL, _bc_state(complete=True))
...
async def test_sell_graduated_token_error(tmp_keystore):
    ...
    with (
        ...
        _setup_mocks(MockClient, MockIDL, _bc_state(complete=True), side="sell")
